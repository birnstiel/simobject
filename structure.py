...
class AbstractGroup(object):
    ...
    def _constructupdater(self, func=None, updater=None):
        ...
        if (updater and func):
            raise ValueError(
                "Giving both, <func> and <updater>, is ambiguous.")
        if updater:
...
class Group(AbstractGroup):
    ...
    def addfield(self, name, value, func=None, updater=None, description=None, constant=False):
        ...
        [1 1 1 1 1]
        """
        self._fields[name] = Field(self._simulation, value, func=func,
                                   updater=updater, description=description, constant=constant)
    ...
    def addgroup(self, name, func=None, updater=None, description=None):
        ...
        self.__dict__[name] = Group(
            self._simulation, func=func, updater=updater, description=description)
    ...
    def __repr__(self):
        ...
        for key in sorted(self.__dict__.keys(), key=str.casefold):
            ...
            if type(val) is Group:
                s += "{:11s}{:7s}: {:12s} {}\n".format(
                    "", "Group", name, "("+val._description+")" if val._description else "")
            elif type(val) is Field:
                s += "{:11s}{:7s}: {:12s} {}\n".format("    Const. " if val._constant else "",
                                                       "Field", name, "("+val._description+")" if val._description else "")
            else:
                s += "{:11s}{:7s}: {:12s}\n".format("",
